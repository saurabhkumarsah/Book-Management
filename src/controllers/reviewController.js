...
export const addReview = async (req, res) => {
    try {
        ...
        if (!rating)
            return res
                .status(400)
                .json({ status: false, message: "Rating is missing" });
        if (!isRating(rating))
            return res
                .status(400)
                .json({
...
export const updateReview = async (req, res) => {
    try {
        ...
        if (req.body.bookId)
            return res.status(401).json({ status: false, message: "Not updatable" });
        ...
        if (!reviewId)
            return res
                .status(400)
                .json({ status: false, message: "Review ID is missing" });

        const dbBook = await bookModel
            .findOne({ _id: bookId, isDeleted: false })
            .select({ ISBN: 0, __v: 0 });
        const dbReview = await reviewModel.findOne({
            _id: reviewId,
            isDeleted: false,
        });
        if (!dbBook)
        ...
        const allReviewData = await reviewModel
            .find({ bookId: bookId })
